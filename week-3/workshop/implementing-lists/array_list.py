...
class ArrayList(List[T]):
    ...
    def delete_at_index(self, index: int) -> T:
        """ Moves self[j+1] to self[j] if j>index, returns old self[index].i
        Do shuffling by means of self.__shuffle_left().
        :pre: index is 0 <= index < len(self) - this is checked by __getitem__() !
        :complexity: In the best-case where the index is the last position, thei
                        loop does not start, so O(1).
                     In the worst-case where the index is the head, the loop 
                        runs fully, so O(n) where n is the length of self.array
        """
